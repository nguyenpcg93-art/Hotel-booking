...
const RoomDetails = () => {
    ...
    const checkAvailability = async ()=>{
        try {
            if(checkInDate >= checkOutDate){
                toast.error('CheckIn should be less than CheckOut')
                return;
            }
    ...
    //onSubmitHandler function to check available & book room
    const onSubmitHandler = async (e)=>{
        try {
            ...
            if(!isAvailable){
                ...
                const {data} = await axios.post('/api/bookings/book', {room: id, guests, checkInDate, checkOutDate, paymentMethod: "Pay at hotel"}, {headers: {Authorization: `Bearer ${await getToken()}`}})
                console.log("CHECK AVAIL RESPONSE:", data)
                if (data.success){
        ...
        {/* Room Details */}
            ...
            <div className='mt-25 space-y-4'>
                ...
                <div className='max-w-3xl border-y border-gray-300 my-15 py-10 text-gray-500'>
                    <p>Implement responsive and multi-level dropdown menus with Tailwind CSS. These Tailwind CSS Dropdown Menus are easy to use and customizable, allowing you to create a wide range of dropdown styles that suit your application's design. Tailwind CSS makes it easy to design and customize dropdown menus.</p>
                </div>
            {/* Host By */}
