...
const AllRooms = () => {
    ...
    //Function to sort room base on sort option
    const sortRooms = (a, b)=>{
    ...
    //Fillter and sort rooms base on selected
    ...
    <div className='flex flex-col-reverse lg:flex-row items-start justify-between pt-28 md:pt-35 px-4 md:px-16 lg:px-24 xl:px-32'>
        <div>
            <div className='flex flex-col items-start text-left'>
                ...
                <p className='text-sm md:text-base text-gray-500/90 mt-2 max-w-174'>Lorem Ipsum Implement responsive and multi-level dropdown menus with Tailwind CSS. These Tailwind CSS Dropdown Menus are easy to use and customizable, allowing you to create a wide range of dropdown styles that suit your application's design. Tailwind CSS makes it easy to design and customize dropdown menus.                                                     </p>
            </div>

        {filteredRooms.map((room)=>(
            <div key={room._id} className='flex flex-col md:flex-row items-start py-10 gap-6 border-b border-gray-300 last:pb:30 last:border-0'>
                <img onClick={()=> {navigate(`/rooms/${room._id}`); scrollTo(0, 0)}} src={room.images[0]} alt="hotel-img" title="View Room Detail" className='max-h-65 md:w-1/2 rounded-xl shadow-lg object-cover cursor-pointer' />            
